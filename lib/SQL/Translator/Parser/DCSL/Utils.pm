...
# returns ($new_text, @files_to_delete) - () if no update
sub _update_class_from_file {
    my ($self, $class, $text, $is_schema) = @_;

    my $filename = $self->_get_dump_filename($class);

    my $ignore_existing = 0;
    my @files_to_delete;
    if (-f $filename && $self->really_erase_my_files && !$self->dry_run) {
        warn "Ignoring existing file '$filename' due to "
            . "'really_erase_my_files' setting\n" unless $self->quiet;
        $ignore_existing = 1;
    }

    my ($old_gen, $old_md5, $old_ver, $old_ts, $old_custom)
        = $ignore_existing ? () : $self->_parse_generated_file($filename);

    if (! $old_gen && (!$ignore_existing and -f $filename)) {
        croak "Cannot update from '$filename' without 'really_erase_my_files',"
    ...
    if (my $renamed_class = $self->_upgrading_classes->{$class}) {
        ...
        if (-f $old_filename) {
            ...
            push @files_to_delete, $old_filename unless $self->dry_run;
        }
    }
    ...
    # Append its MD5 sum
    $text .= Digest::MD5::md5_base64(encode 'UTF-8', $text) . "\n";

    # Add anything loaded via external partial class file in @INC
    $text .= qq|$_\n|
        for @{$self->{_ext_storage}->{$class} || []};

    # Add any custom content the user has added
    $text .= $custom_content;

    return ($text, @files_to_delete);
}
